...
func createTestCascadeDeleteObjectsSQLite() interface{} {
	recreateTestCascadeTablesSQLite()

	var i uint64 = 1
	company := &TestCompany{Name: "Company", ID: i}
	_ = testCRUDSQLite.Save(context.Background(), company, SaveOptions{})

	for j := 1; j < 3; j++ {
		jU := uint64(j)
		employee := &TestEmployee{Name: "Employee", ID: i*10 + jU, TestCompanyID: i}
		_ = testCRUDSQLite.Save(context.Background(), employee, SaveOptions{})

		for k := 1; k < 3; k++ {
			kU := uint64(k)
			_ = testCRUDSQLite.Save(context.Background(), &TestCreditCard{
				ID: i*100 + jU*10 + kU, Number: "number", TestEmployeeID: i*10 + jU,
			}, SaveOptions{})
			_ = testCRUDSQLite.Save(context.Background(), &TestComment{
				ID: i*100 + jU*10 + kU, Comment: "comment", TestEmployeeID: i*10 + jU,
			}, SaveOptions{})
		}
	}
...
func TestSQLite_DeleteCascade(t *testing.T) {
	p := createTestCascadeDeleteObjectsSQLite()

	check := func(query string, args ...interface{}) int {
		var cnt int
		if err := testDBSQLite.QueryRow(query, args...).Scan(&cnt); err != nil {
			t.Fatalf("SQLite cascade count query failed: %s", err.Error())
		}
		return cnt
	}

	if check("SELECT COUNT(*) FROM test_company") != 1 {
		t.Fatalf("SQLite cascade: wrong initial parent count")
	}
	if check("SELECT COUNT(*) FROM test_employee WHERE id IN (11,12) AND test_company_id != 0") != 2 {
		t.Fatalf("SQLite cascade: wrong initial employee count")
	}
	if check("SELECT COUNT(*) FROM test_credit_card WHERE id IN (111,112,121,122) AND test_employee_id != 0") != 4 {
		t.Fatalf("SQLite cascade: wrong initial credit-card count")
	}
	if check("SELECT COUNT(*) FROM test_comment WHERE id IN (111,112,121,122) AND test_employee_id != 0") != 4 {
		t.Fatalf("SQLite cascade: wrong initial comment count")
	}

	err := testCRUDSQLite.Delete(context.Background(), p, DeleteOptions{})
	if err != nil {
		t.Fatalf("SQLite cascade Delete failed: %s", err.Error())
	}

	if check("SELECT COUNT(*) FROM test_company") != 0 {
		t.Fatalf("SQLite cascade: parent was not deleted")
	}
	if check("SELECT COUNT(*) FROM test_employee WHERE id IN (11,12)") != 0 {
		t.Fatalf("SQLite cascade: children (employees) were not deleted")
	}
	if check("SELECT COUNT(*) FROM test_credit_card WHERE id IN (111,112,121,122)") != 0 {
		t.Fatalf("SQLite cascade: grand-children (credit cards) were not deleted")
	}
	if check("SELECT COUNT(*) FROM test_comment WHERE id IN (111,112,121,122) AND test_employee_id=0") != 4 {
		t.Fatalf("SQLite cascade: grand-children (comments) were not updated to zero employee_id")
	}
}
